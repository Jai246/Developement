...
(async function fn() {
    ...
    // title yt-simple-endpoint style-scope ytd-playlist-video-renderer or (a[id = "video-title"])
    // time style-scope ytd-thumbnail-overlay-time-status-renderer or span[id = 'text']
    
    // there are hidden spinners in this page
    // I disabled the wifi and then found the spinner by inspecting on it
    // spinner ->  class="circle style-scope tp-yt-paper-spinner"
    ...
    let videosArr = [];
    for (let i = 0; i < videoNameElementList.length-5; i++) {
